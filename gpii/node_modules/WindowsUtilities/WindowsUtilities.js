...
windows.API_constants = {
    ...
    MAX_PATH: 260,
    // https://autohotkey.com/board/topic/55150-class-structfunc-sizeof-updated-010412-ahkv2/
...
windows.API_constants.TH32CS_SNAPHEAPLIST = TH32CS_SNAPHEAPLIST;
windows.API_constants.TH32CS_SNAPMODULE = TH32CS_SNAPMODULE;
windows.API_constants.TH32CS_SNAPPROCESS = TH32CS_SNAPPROCESS;
windows.API_constants.TH32CS_SNAPTHREAD = TH32CS_SNAPTHREAD;
...
windows.MODULEENTRY32 = new Struct([
    [t.DWORD, "dwSize"],
    [t.DWORD, "th32ModuleID"],
    [t.DWORD, "th32ProcessID"],
    [t.DWORD, "GlblcntUsage"],
    [t.DWORD, "ProccntUsage"],
    [t.BYTE_PTR, "modBaseAddr"],  // declared as BYTE* in documentation
    [t.DWORD, "modBaseSize"],
    [t.HKL, "hModule"],
    [arrayType("char", c.MAX_MODULE_NAME32 + 1), "szModule"],
    [arrayType("char", c.MAX_PATH), "szExePath"]
]);

// https://msdn.microsoft.com/en-us/library/windows/desktop/ms686735(v=vs.85).aspx
windows.THREADENTRY32 = new Struct([
    [t.DWORD, "dwSize"],
    [t.DWORD, "cntUsage"],
    [t.DWORD, "th32ThreadID"],
    [t.DWORD, "th32OwnerProcessID"],
    [t.LONG, "tpBasePri"],
    [t.LONG, "tpDeltaPri"],
    [t.DWORD, "dwFlags"]
]);
...
/**
 * Returns a snapshot of the currently running processes, their modules, and
 * their threads.  See:
 * https://msdn.microsoft.com/en-us/library/windows/desktop/ms686701(v=vs.85).aspx
 *
 * The structure of a procees object is:
 * { pid, execFile, parentPid, modules (array), threads (array) }.
 *
 * See also windows.listProcessModules() and windows.listProcessThreads().
 *
 * @return {Array} An array windows process objects.
 */
windows.processesSnapshot = function () {
    var processes = [];
    ...
    while (hRes) {
        var aProcess = {};
        aProcess.pid = pEntry.th32ProcessID;
        aProcess.parentPid = pEntry.th32ParentProcessID;
        var buf = new Buffer(pEntry.szExeFile);
        aProcess.execFile = ref.readCString(buf, 0);
        windows.moduleNameAndPath (aProcess);
        aProcess.threads = windows.listProcessThreads(aProcess.pid);
...
/**
 * Returns command name and full path for the process from its first module.
 * https://msdn.microsoft.com/en-us/library/windows/desktop/ms686701(v=vs.85).aspx
 *
 * If the process's first module cannot be read properly, the name and/or path
 * are empty strings.
 *
 * @param {Object} process  The process object whose pid member is initialized.
 *                          Used to store the name and fullPath.
 */
windows.moduleNameAndPath = function (process) {
    var nameAndPath = {};
    var module = new windows.MODULEENTRY32();
    module.dwSize = windows.MODULEENTRY32.size;

    var hModuleSnapShot = windows.kernel32.CreateToolhelp32Snapshot(c.TH32CS_SNAPMODULE, process.pid);
    if (windows.kernel32.Module32First(hModuleSnapShot, module.ref())) {
        var buf = new Buffer(module.szModule);
        process.name = ref.readCString(buf, 0);

        buf = new Buffer(module.szExePath);
        process.fullPath = ref.readCString(buf, 0);
    }
    else {
        process.name = "";
        process.fullPath = "";
...
/**
 * Returns a list of module information for the given process.  See:
 * https://msdn.microsoft.com/en-us/library/windows/desktop/ms686701(v=vs.85).aspx
 *
 * The structure of a module object is:
 * { name, fullPath, pid }.
 *
 * @param {Number} pid The id of the process.
 * @return {Array} An array of module objects.
 */
windows.listProcessModules = function (pid) {
    var modules = [];
    var hModuleSnapShot = windows.kernel32.CreateToolhelp32Snapshot(c.TH32CS_SNAPMODULE, pid);
    var mEntry = new windows.MODULEENTRY32();

    mEntry.dwSize = windows.MODULEENTRY32.size;
    var hRes = windows.kernel32.Module32First(hModuleSnapShot, mEntry.ref());
    while (hRes) {
        var aModule = {};
        aModule.pid = pid;
        var buf = new Buffer(mEntry.szModule);
        aModule.name = ref.readCString(buf, 0);
        buf = new Buffer(mEntry.szExePath);
        aModule.fullPath = ref.readCString(buf, 0);
        modules.push(aModule);
        hRes = windows.kernel32.Module32Next(hModuleSnapShot, mEntry.ref());
    }
    windows.kernel32.CloseHandle(hModuleSnapShot);
    return modules;
};

/**
 * Returns a list of threads for the given process.  See:
 * https://msdn.microsoft.com/en-us/library/windows/desktop/ms686701(v=vs.85).aspx
 *
 * The structure of a thread object is:
 * { tid, basePriority, deltaPriority }.
 *
 * @param {Number} pid The id of the process.
 * @return {Array} An array of thread objects.
 */
 windows.listProcessThreads = function (pid) {
    var threads = [];
    ...
    while (hRes) {
        ...
        if (tEntry.th32OwnerProcessID === pid) {
            ...
            threadInfo.basePriority = tEntry.tpBasePri;
            threadInfo.deltaPriority;
            threads.push(threadInfo);
        }
        hRes = windows.kernel32.Thread32Next(hThreadSnapShot, tEntry.ref());
    }
    windows.kernel32.CloseHandle(hThreadSnapShot);
    return threads;
};
