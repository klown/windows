...
namespace gpii.windows.processes.tests
{
    public class GetProcessesTests
    {
        ...
        private Exception testError = null;
        private GetProcesses getProcesses = new GetProcesses();
        ...
        public async Task<object> TestValidCommandName(dynamic input)
        {
            var runAssertions = (Func<object, Task<object>>)input.assert;
            ...
            var result = (string)await runAssertions(testResults.ToArray());
            testResults.Clear();
            return result;
        }

        public async Task<object> TestInvalidCommandName(dynamic input)
        {
            var runAssertions = (Func<object, Task<object>>)input.assert;
            object[] processes = null;
            ...
            asserts.Add(new {
                message = "No process with name 'Txe243'",
                expected = 0,
                actual = (processes == null ? 0 : processes.Length),
                assert = "assertEquals",
                error = testError
        ...
        public async Task<object> TestValidPid(dynamic input)
        {
            var runAssertions = (Func<object, Task<object>>)input.assert;
            int thisPid = (int) input.pid;
            object[] processes = null;

            // Test getProcesses(<pid>) using node's pid (expected that this
        ...
        public async Task<object> TestInvalidPid(dynamic input)
        {
            var runAssertions = (Func<object, Task<object>>)input.assert;
            object[] processes = null;
        ...
        public async Task<object> TestNoArguments(dynamic input)
        {
            var runAssertions = (Func<object, Task<object>>)input.assert;
            object[] processes = null;
