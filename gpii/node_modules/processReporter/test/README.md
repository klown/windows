...
`edge.js` provides a way to call back into `node` from C# code by passing a function reference as a member of the `dataForCSharp` marshalled and passed to C#.  This ability opens the door to a way executing tests within the C# code itself, but use `jqUnit` to evaluate the test results.
...
The main difference is where the test scenarios are run.  In the first case, while the code tested is implemented in C#, that is a black box where only the interface between `node` and C#/.NET is evaluated.  All of the specifics of what happens in C# is opaque.  In the second case, since the tests are set up within C#/.NET, a finer degree of testing is possible since the test is executed in C# itself.
