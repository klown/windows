...
What is missing currently is any unit testing of the C# code.  As (if) GPII uses C# more and more, unit testing will become necessary.  This README is a exploration on how unit testing might be accomplished.
...
The second approach uses `edge.js`'s ability to call back into `node` from within C#.  Here the C# code runs the test scenario, collects the results, and then passes them back to `node` where the results are evaluated using `jqUnit`.
...
`dotNetProcesses()` is well-defined in terms of its inputs and outputs.  As such, one could write unit tests using `jqUnit` and simply treat `dotNetProcesses()` as any other JavaScript function, running the following test scenarios and their expected outcomes:
...
`edge.js` provides a way to call back into `node` from C# code by passing a function reference as a member of the `dataForCSharp` marshalled and passed to C#.  This ability opens the door to a way executing tests within the C# code itself, and using `jqUnit` to evaluate the test results.
...
```
`fluid.each()` iterates over an array of such "test fixture" `Object`s, calling `jqUnit.test()` for each fixture.  If the `error` is non-null, `jqUnit.fail()` is called.  Otherwise the specified `jqUnit.assertX()` function is run:
```
...
jqUnit.test(
...
There is a JavaScript `procTests.testX()` function created by `edge.func()` for each C# `GetProcessesTests.TestX()` method.  The test is initiated in node by running each of the `procTests.testX()` generated functions to execute the test scenarios defined by the C# method.  The `procTests.runAssertions()` function is passed into C# so that it can call back into `node` when the C# code has finished executing the test and collected the results:
```
...
Which is better?  I think it depends on the need for how finely tuned testing.  If the C# implementation is simple and nearly one-to-one with its JavaScript invocation, then the first method is sufficient.
