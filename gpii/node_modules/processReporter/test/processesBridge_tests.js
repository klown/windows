...
(function () {
    ...
    var killer = fluid.registerNamespace("gpii.windows");
...
*/
    ...
    // Windows (because must specify command with ".exe" suffix.
    jqUnit.test(
    ...
    // Windows, since specifying a Windows command for the test.
    jqUnit.test(
    ...
    jqUnit.test(
        ...
        function () {
            ...
            var notePadProcInfo = processesBridge.initProcInfoNotRunning("notepad.exe");
    ...
    // Windows, since using windows specific command
    jqUnit.test(
    ...
    // Windows, using 'magnify.exe' as the process test case.
    jqUnit.test(
        "Test gpii.processes against launching notepad  (JAWS?).",
        function () {
            debugger;
            var magnifyProcInfo = processesBridge.findFirstProcessByCommand("magnify.exe");
            ...
            // Give Magnfiy.exe some time to start and initialize itself.  Then look
            // for the 'magnify.exe' process.
            ...
            if (!wasRunning) {
                killer.killProcessByName("Magnify.exe");
            }
            procTests.waitMsec(500);
            if (magnifyProcInfo !== null) {
                var magnifyProcNewState =
                    processesBridge.findProcessByPid(magnifyProcInfo.pid);
